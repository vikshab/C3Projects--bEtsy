  ...
  describe "methods" do
    ...
    context "order_price" do
      ...
      it "has a price based on its order items" do
        expect(@order.order_price).to eq(@item.total_item_price)
        ...
        @order.reload
        expect(@order.order_price).to eq(@item.total_item_price + item2.total_item_price)
      ...
      it "can have a price based on a single seller's items" do
        expect(@order.order_price(@seller.id)).to eq(@item.total_item_price)
